    ...
    const songs=[
      { src:"assets/1800.mp3", cover:"assets/cover.jpg",  title:"1-800",                artist:"bbno$, Ironmouse",                         bpm:132 },
      { src:"assets/mbof.mp3", cover:"assets/cover2.jpg", title:"Music Box of Fate",    artist:"StarlightDaryl, Ironmouse, WUNDER RIKU",     bpm:116 },
      { src:"assets/c4m.mp3",  cover:"assets/cover3.jpg", title:"Cry For Me (WA WA WA)",artist:"Ironmouse, shirobeats, HalaCG, Bubi",         bpm:124 }
    ];
    let currentIndex=0;
    ...
    // === Music-reactive blob pulse (BPM + energy) ===
    ...
    function getEnergy(){
      if (analyser && data) {
        let sum=0, n=0;
        for (let i=2; i<18; i++){ sum += data[i]; n++; }
        return (sum / (n*255)); // 0..1
      }
      return 0.5; // fallback on file://
    }
    function pulseLoop(){
      const t = audio.currentTime || performance.now()/1000;
      const bpm = songs[currentIndex].bpm;
      const beatPeriod = 60 / bpm;
      const beat = Math.sin((t / beatPeriod) * Math.PI * 2);
      const beatAmp = (beat + 1) / 2; // 0..1
      const energy = getEnergy();

      const base = 1;
      const eAmt = 0.06; // energy contribution
      const bAmt = 0.08; // beat contribution

      blobs.forEach((el,i)=>{
        const jitter = 0.015 * Math.sin(t*0.7 + offsets[i]);
        const scale = base + (energy * eAmt) + (beatAmp * bAmt) + jitter;
        el.style.setProperty('--pulse', scale.toFixed(3));
        el.style.opacity = 0.28 + 0.10 * beatAmp;
      });

      rafPulse = requestAnimationFrame(pulseLoop);
    }

    // Kick everything on first user gesture (to satisfy autoplay policies)
